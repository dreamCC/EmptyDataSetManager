...
/*
 该类是基于DZNEmptyDataSet(Github:https://github.com/dzenbot/DZNEmptyDataSet)的基础上开发的。使用该类可能需要对DZNEmptyDataSet有所了解。
 
 */

@class EmptyDataSetManager;
typedef void(^EmptyDataSetTapView)(__kindof UIScrollView *scrollView, UIView *tapView, EmptyDataSetManager *manager);
typedef void(^EmptyDataSetTapButton)(__kindof UIScrollView *scrollView, UIButton *button, EmptyDataSetManager *manager);

@interface EmptyDataSetManager : NSObject<DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@interface EmptyDataSetManager (Appearance)
